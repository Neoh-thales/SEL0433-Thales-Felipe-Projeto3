...
Os maiores desafios apareceram na **Parte 2.2**. O grande problema que precisávamos evitar era fazer com que o processamento do Wi-Fi ou a leitura do sensor ultrassônico não "roubassem" o tempo do motor, o que faria a cancela engasgar. Nós resolvemos isso de duas formas: usando **Timers dedicados de hardware** e montando uma **Lógica de Estados**. Essa lógica permitiu não usar o `delay()` e passou a usar o `millis()`, que funciona como um cronômetro que não atrapalha o resto do sistema.
